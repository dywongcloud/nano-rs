...
async fn test_isolate_id_changes_after_oom_recovery() {
    ...
    // Create VFS backend for the test
    let vfs_backend = Arc::new(MemoryBackend::new());
    
    // Create the worker pool with memory limits
    let hostname = "oom-test.local";
    let entrypoint = "/test/oom_handler.js";
...
async fn test_request_tracing_combo() {
    ...
    let entrypoint = "/test/handler.js";
